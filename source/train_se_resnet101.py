...
from utils.average_precision_calculator import AveragePrecisionCalculator
...
def get_path(rel_path, config):
    return os.path.join(config['TRAINING']['ARTIFACTS_ROOT'], rel_path)
...
def validate(model, val_dataset, batch_size, top_k):
    model.eval()
    ...
    val_loader = DataLoader(val_dataset, batch_size, num_workers=4, collate_fn=lambda X: X)
    for samples in val_loader:
        ...
        with torch.no_grad():
            ...
            loss = model(input_tensor, labels)

        loss_meter.update(loss.item(), len(samples))

        cur_actuals = np.zeros_like(pred_labels, dtype=np.bool)
        for i in range(len(pred_labels)):
            assert len(labels[i]) <= top_k
            total_num_positives += len(labels[i])
            for label in labels[i]:
                cur_actuals[i] = np.logical_or(cur_actuals[i], pred_labels[i] == label)

        predictions.extend(pred_scores.flatten().tolist())
        actuals.extend(cur_actuals.astype(np.uint8).flatten().tolist())

    gap = AveragePrecisionCalculator.ap_at_n(predictions, actuals, n=None, total_num_positives=total_num_positives)
    return loss_meter.avg, gap
...
def main():
    ...
    fh = logging.FileHandler(get_path('train.log', config))
    ...
    val_transform = transforms.Compose([transforms.Resize(256),
                                        ...
                                        transforms.ToTensor(),
                                        normalize])
    train_dataset = LabeledImages(config['DATASET']['TRAIN_LIST_PATH'], config['DATASET']['RESIZED_IMAGES_ROOT'],
                                  train_transform)
    val_dataset = LabeledImages(config['DATASET']['VAL_LIST_PATH'], config['DATASET']['RESIZED_IMAGES_ROOT'],
                                  val_transform)
    logger.info('Train size: {}. Val size: {}.'.format(len(train_dataset), len(val_dataset)))
    ...
    for epoch in range(start_epoch, config['TRAINING']['N_EPOCH'] + 1):
        adjust_learning_rate(optimizer, epoch, config['TRAINING']['INITIAL_LR'], config['TRAINING']['LR_DECAY'],
                             config['TRAINING']['LR_STEP'])
        logger.info('Start epoch {} / {}.'.format(epoch, config['TRAINING']['N_EPOCH']))
        val_loss, val_gap = validate(model_with_loss, val_dataset, config['TRAINING']['BATCH_SIZE'],
                                     config['VALIDATION']['TOP_K'])
        logger.info('Val loss: {}. Val GAP: {}.'.format(val_loss, val_gap))
        writer.add_scalar('val_loss', val_loss, iteration)
        writer.add_scalar('val_gap', val_gap, iteration)
